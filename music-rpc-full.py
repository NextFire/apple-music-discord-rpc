...
def music_sessions_as():
    return int(subprocess.run(["osascript",
                               ...
                               "-e", "end tell"], capture_output=True, text=True).stdout)
...
def music_state_as():
    return subprocess.run(["osascript",
                           ...
                           "-e", "end if",
                           "-e", "end tell"], capture_output=True, text=True).stdout.rstrip()
...
def music_info_as():
    return subprocess.run(["osascript",
                           ...
                           "-e", "end tell"], capture_output=True, text=True).stdout.rstrip().split(', ')
    ...
    if state in ('playing', 'paused'):
        ...
        try:
            RPC.update(
                ...
                small_image=state,
                small_text=f'{state.capitalize()}『{infos[0]}』by {infos[1]} ({infos[3]})',
                details=infos[0],
                state=f'{infos[1]} — {infos[2]} ({infos[3]})',
                end=time.time() + float(infos[4]) - float(infos[5]) if state == 'playing' else None)
        except IndexError:
            RPC.update(large_image=APPICON,
                       large_text=APPICON_DESC,
                       small_image=state,
